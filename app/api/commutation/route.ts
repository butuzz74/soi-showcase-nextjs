...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({
      products: commutations,
      totalPages: Math.ceil(totalCommutations / perPage),
      currentPage: page,
      totalProducts: totalCommutations,
    });
  } catch (error) {
  ...
  try {
    ...
    if (body) {
      if (body.access) {
        ...
        await connectDB();
        const newCommutation = await Commutation.create({
          ...body,
          access: accessBoolean,
        });
        return NextResponse.json(newCommutation);
      }
      await connectDB();
      const newCommutation = await Commutation.create({ ...body });
      return NextResponse.json(newCommutation);
    }
  } catch (error) {
    return NextResponse.json({ message: 'Ошибка сервера' }, { status: 500 });
