...
export async function generateMetadata({
  ...
  const data = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL}/api/commutation/${id}`,
    { cache: 'no-store' }
  );
  const display: ProductType = await data.json();
  if (!display) {
    return {
      title: 'Equipments Not Found',
      description: 'Details about this equipments are not available.',
    };
  }
...
async function CommutationPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
    ...
    <div className="mx-auto my-8 flex h-full w-full flex-col items-center justify-between rounded-lg border border-gray-200 bg-white shadow-lg">
      ...
      <div className="mb-8 flex justify-center">
        {session ? (
          <NavigationButton
            text="Редактировать"
            customAction={`/edit?currentProduct=commutation&id=${display._id}`}
          />
        ) : null}
...
export default CommutationPage;
