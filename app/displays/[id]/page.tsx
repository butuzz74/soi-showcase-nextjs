...
export async function generateMetadata({
  ...
  const data = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL}/api/display/${id}`,
    { cache: 'no-store' }
  );
  ...
  if (!display) {
    return {
      ...
      description: 'Details about this display are not available.',
    };
  }
    ...
    <div className="mx-auto my-8 flex h-full w-full flex-col items-center justify-between rounded-lg border border-gray-200 bg-white shadow-lg">
      ...
      <div className="mb-8 flex justify-center">
        {session ? (
          <NavigationButton
            text="Редактировать"
            customAction={`/edit?currentProduct=display&id=${display._id}`}
          />
        ) : null}
